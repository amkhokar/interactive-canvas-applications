...
(function () {
  ...
  var whiteMarker = function () {
    ...
    space.add({
      ...
      animate: (time, ftime) => {
        form.fillOnly("#fff").point(space.pointer, 100, "circle");
      }
    ...
    space.add({
      animate: (time, ftime) => {
        ...
        for (let i = 4, len = pts.length; i < len; i += 3) {
          pts[i].rotate2D(((i % 7 === 0) ? 0.002 : -0.003), pts[i - 1]);

          // align the other control pt by extrapolating its corresponding position
          pts[i - 2].to(Geom.interpolate(pts[i], pts[i - 1], 2));
        }

        form.strokeOnly("#f03", 10, "round").line(Curve.bezier(pts));
        form.strokeOnly("rgba(255,255,255,.3)", 1).line(pts);
  ...
  const SoundPanels = {};
  const MainSong = new Audio("./audio/instrumental2.mp3");
